        ...
        <Formik initialValues={initialValues} validationSchema={AuthFormSchema} onSubmit={submitHandler}>
            {
                ({ isSubmitting }) => (
                    <Form className=" w-1/2 mx-auto">
                        <h1 className=" text-center font-bold text-4xl my-4 text-teal-600 ">{ isLogin ? "Login" : "Register" } </h1>
                        {
                        ...
                        <button className=' text-white bg-teal-600 py-3 font-medium w-full text-center' type='submit' disabled = {isSubmitting}>
                            {isLogin ? `${isSubmitting ? "Submitting..." : "Login"}` : `${isSubmitting ? "Submitting..." : "Register"}`} </button>
                    </Form>
                )
            }
        </Formik>
